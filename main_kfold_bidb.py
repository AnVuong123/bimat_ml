...
def run_bidb(seed, gpu_id,k):
        ...
        #     data_options=[f"data/cls-bidb-train/fold_{k}"]
        #     df=pd.read_csv(f"data/cls-bidb-train/fold_{k}/id_prop.csv")
        # else:
        data_options=[f"data/reg-bidb-train"]
        print(data_options)
        #args.train_size =len(df)
        #print(args.train_size)
        dataset = CIFDataBiDB(*data_options,k=k)
        collate_fn = collate_pool_bidb
        ...
        if args.task == 'classification':
            ...
            _, sample_target, _ , _, = collate_pool_bidb(sample_data_list)
            normalizer = Normalizer(sample_target)

        # build model
        graph, _, _, _= dataset[0]
        atom, nbr, idx = graph
        ...
        model = BiDBlayerCrystalGraphConvNet(
                    orig_atom_fea_len,
                    nbr_fea_len,
        ...
        for epoch in range(args.start_epoch, args.epochs):
            ...
            # train for one epoch
            train_bidb(train_loader, model, criterion, optimizer, epoch, normalizer,mono=mono)
            ...
            if (epoch + 1) % 100 == 0:
                if args.task == 'regression':
                    if mono==True:
                        last_ckpt_path = f'pre-trained/model_last_bidb_monobg_{epoch+1}ep_{args.lr}_128b_kfold_{k}.pth.tar'
                    else:
                        last_ckpt_path = f'pre-trained/model_last_bidb_{epoch+1}ep_{args.lr}_128b_kfold_{k}.pth.tar'
                    torch.save({
                        'epoch': epoch+1,
                    ...
                    if mono==True:
                        last_ckpt_path = f'pre-trained/model_last_bidb_stable_monobg_{epoch+1}ep_{args.lr}_128b_kfold_{k}.pth.tar'
                    else:
                        last_ckpt_path = f'pre-trained/model_last_bidb_stable_{epoch+1}ep_{args.lr}_128b_kfold_{k}.pth.tar'
                    torch.save({
                        'epoch': epoch+1,
                        'state_dict': model.state_dict(),
                        'best_mae_error': best_mae_error,
...
def train_bidb(train_loader, model, criterion, optimizer, epoch, normalizer,mono):
    ...
    for i, (input, target,mono_bg, mat_name) in enumerate(train_loader):
        ...
        if args.cuda:
            ...
            #atom = atom.cuda(non_blocking=True)
            #nbr  = nbr.cuda(non_blocking=True)
            #idx  = idx.cuda(non_blocking=True)
            #crys = [c.cuda(non_blocking=True) for c in crys]
        ...
        # 4) Forward pass
        # ================================
        output = model(*input_var,mono_bg,mono=mono)
...
def main():
    ...
    for idx, seed in enumerate(list_seed):
        gpu_id = idx % num_gpus
        p = mp.Process(target=run_bidb, args=(seed, gpu_id, k))
        p.start()
        processes.append(p)
        k=k+1
