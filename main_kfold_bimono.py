...
def run_bimono(seed, gpu_id,k):
        ...
        data_options=[f"data/reg-{dataset}-train/"]
        dataset = CIFDataMonoDB(*data_options,k=k)
        ...
        if args.task == 'classification':
            ...
            _, sample_target, _ , _, _, _ = collate_pool_monodb(sample_data_list)
        ...
        for epoch in range(args.start_epoch, args.epochs):
            ...
            # train for one epoch
            train_bimono(train_loader, model, criterion, optimizer, epoch, normalizer,mono)
            ...
            if (epoch + 1) % 500 == 0:
                if args.task == 'regression':
                    if mono==True:
                        last_ckpt_path = f'pre-trained/model_last_bimono_monobg_{epoch+1}ep_{args.lr}_128b_kfold_{k}.pth.tar'
                    else:
                        last_ckpt_path = f'pre-trained/model_last_bimono_{epoch+1}ep_{args.lr}_128b_kfold_{k}.pth.tar'
                    torch.save({
                        'epoch': epoch+1,
                    ...
                    if mono==True:
                        last_ckpt_path = f'pre-trained/model_last_bimon2_or_stable_monobg_{epoch+1}ep_{args.lr}_128b_kfold_{k}.pth.tar'
                    else:
                        last_ckpt_path = f'pre-trained/model_last_bimon2_or_stable_{epoch+1}ep_{args.lr}_128b_kfold_{k}.pth.tar'
                    torch.save({
                        'epoch': epoch+1,
                        'state_dict': model.state_dict(),
                        'best_mae_error': best_mae_error,
...
def train_bimono(train_loader, model, criterion, optimizer, epoch, normalizer,mono):
    ...
    for i, (input, target,mono_bg, mono_name, mat_name, config_vector) in enumerate(train_loader):
        ...
        # 4) Forward pass
        # ================================
        output = model(*input_var,config_vector, mono_bg,mono)
...
def main():
    ...
    for idx, seed in enumerate(list_seed):
        gpu_id = idx % num_gpus
        p = mp.Process(target=run_bimono, args=(seed, gpu_id, k))
        p.start()
        processes.append(p)
        k=k+1
...
def main2():
    ...
    for k, seed in enumerate(list_seed):
        print(f"Running job {k} with seed {seed}")
        run_bimono(seed,0, k)   # gọi trực tiếp, không process, không GPU
