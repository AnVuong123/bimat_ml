...
def main():
    ...
    for k in range(0,4):
        ...
        print(k)
        mono=False
        model_path=f"pre-trained/model_bidb_kfold_{k}.pth.tar"
        model_checkpoint = torch.load(model_path,
                                    map_location=lambda storage, loc: storage)
        model_args = argparse.Namespace(**model_checkpoint['args'])
        cls=False
        cifpath=f"data/reg-bidb-test"
        
        dataset = CIFDataBiDB(cifpath,k=k)
       ...
       #         print(i, type(x))
        ...
        if model_args.task == 'classification':
            c=1
            accuracy, precision, recall,f1=validate(test_loader, model, criterion, normalizer,c, test=True,mono=mono)
    ...
    if model_args.task == 'classification':
        ...
        print(model_path)
        print(list_mae)
        print(f"MAE  : {np.mean(list_mae):.2f} ± {np.std(list_mae):.2f}")
        print(f"MSE  : {np.mean(list_mse):.2f} ± {np.std(list_mse):.2f}")
        print(f"RMSE : {np.mean(list_rmse):.2f} ± {np.std(list_rmse):.2f}")
        print(f"R2   : {np.mean(list_r2):.2f} ± {np.std(list_r2):.2f}")



def validate(val_loader, model, criterion, normalizer, c,test, mono):
    ...
    for i, (input, target,mono_bg, mat_name ) in enumerate(val_loader):
